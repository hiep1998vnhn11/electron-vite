...
function logStats(proc, data) {
  ...
  log += chalk.yellow.bold(
    `┏ ${proc} ${config.dev.chineseLog ? 'Đang xử lý' : 'Process'} ${new Array(
      19 - proc.length + 1
    ).join('-')}`
  )
  log += '\n\n'
              ...
              chalk.blue(
                `${
                  config.dev.chineseLog
                    ? 'Đang chuẩn bị quy trình chính, vui lòng đợi ...'
...
function startMain() {
  return new Promise((resolve, reject) => {
    ...
    watcher.on('change', (filename) => {
      // 主进程日志部分
      logStats(`${config.dev.chineseLog ? 'File chính đã thay đổi' : 'Main-FileChange'}`, filename)
    })
...
function electronLog(data, color) {
  if (data) {
    ...
    if (/[0-9A-z]+/.test(log)) {
      console.log(
        chalk[color].bold(
          `┏ ${config.dev.chineseLog ? 'Electron' : 'Electron'} -------------------`
        ) +
          '\n\n' +
          log +
          chalk[color].bold('┗ ----------------------------') +
...
function greeting() {
  ...
  console.log(
    chalk.blue(`${config.dev.chineseLog ? '   Đang chuẩn bị ...' : '  getting ready...'}`) + '\n'
  )
}
